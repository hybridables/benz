...
module.exports = function factory (self, method) {
  ...
  return function _factory_ (fns, exts) {
    ...
    var _ctx = self.option('context')
    var _ext = self.option('extensions')
    var context = isObject(_ctx) ? _ctx : {}
    var extensions = extend(isObject(_ext) ? _ext : {}, exts)

    self.option('context', context)
    self.option('extensions', extensions)
    ...
    if (self.enabled('promise')) {
      return self.option('promisify')(complete)
    }
